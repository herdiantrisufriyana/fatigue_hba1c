shap_waterfall_plot <-
  function(identifier, data, threshold, exp_prob, exp_obs, shap_text_size = 3){
    ...
    data <-
      ...
      mutate(
        ...
        , shap_pos =
          ifelse(shap_pos <= 0, 0, ifelse(shap_pos >= 1, 1, shap_pos))
      )
    ...
    data |>
      ...
      geom_segment(
        aes(
          ...
          , y = shap_start
          , yend = shap_end
          , color = shap_direction
        )
        , linewidth = 9
      ) +
      geom_text(
        aes(label = sprintf("%+0.2f", shap_value), y = shap_pos)
        , hjust = "inward", size = shap_text_size
      ) +
